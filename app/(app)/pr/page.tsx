...
export default function PrStudioPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <h1 className="mb-1 text-2xl font-bold">웹자보 생성</h1>
      <p className="mb-6 text-sm text-slate-500">
        주제를 입력해 AI로 문구를 생성하고(또는 직접 입력하고), 템플릿을 골라 이미지로
        내려받으세요.
      </p>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_420px]">
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <input
              ...
              className="mb-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand focus:ring-1 focus:ring-brand"
            />
            <input
              value={keyMessage}
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="rounded-lg border border-slate-200 px-2 py-2 text-sm outline-none focus:border-brand"
              >
                {TONES.map((t) => (
              ...
              <button
                ...
                className="flex-1 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-dark disabled:cursor-not-allowed disabled:opacity-40"
              >
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="space-y-3 text-sm">
              ...
              <Field label="핵심 문구 (한 줄에 하나)">
                <textarea
                  rows={3}
                  value={content.points.join("\n")}
                  onChange={(e) =>
                    set({ points: e.target.value.split("\n") })
                  }
                  className={inputCls}
                />
              </Field>
              <Field label="안내 문구(CTA)">
                <input
                  value={content.cta}
        ...
        <div className="lg:sticky lg:top-6 lg:self-start">
          ...
          {/* 1080x1350 포스터를 0.37배로 축소 표시 (400x500) */}
          <div
