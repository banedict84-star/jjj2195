...
function parseScheduleRuleBased(utterance) {
  ...
  let title = text
    ...
    // 명령형 표현 제거: "일정 등록해줘", "추가해줘", "잡아줘", "넣어줘" 등
    .replace(/일정(을|좀)?/g, " ")
    .replace(/(등록|추가|저장|예약|입력)\s*(좀)?\s*(해\s*줘|해\s*주세요|해\s*도|하기|해|해줭)?/g, " ")
    .replace(/(잡아|넣어|적어|기록)\s*(줘|주세요|둬|놔)?/g, " ")
    .replace(/(해\s*줘|해\s*주세요|부탁(해|드려요|할게)?|좀)/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  if (!title) title = "일정";
